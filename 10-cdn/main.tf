resource "aws_cloudfront_distribution" "prudhvi" {
  enabled             = true
  ...
  records = [
    {
      ...
      type    = "A"
      alias   = {
        name    = aws_cloudfront_distribution.prudhvi.domain_name
        zone_id = aws_cloudfront_distribution.prudhvi.hosted_zone_id
      }
    }
  ]
}